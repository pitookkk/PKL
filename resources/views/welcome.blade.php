...
<div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="relative w-full overflow-hidden fade-mask">
            <div class="flex w-max">
                ...
                <div class="flex w-max items-center animate-scroll">
                    ...
                    <img src="{{ asset('images/brands/'.strtolower(str_replace(' ', '-', $brand)).'.svg') }}" alt="{{ $brand }}" onerror="this.src='https://via.placeholder.com/150?text={{ $brand }}'" class="h-8 mx-12 grayscale opacity-40 hover:grayscale-0 hover:opacity-100 transition-all duration-300">
                    @endforeach
                </div>
                <div class="flex w-max items-center animate-scroll">
                    @foreach ($brands as $brand)
    ...
    <section class="mb-20 reveal-on-scroll" x-data="{ shown: false }" x-intersect.once="shown = true" :class="{ 'is-visible': shown }">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div class="group bg-white rounded-[2rem] p-4 shadow-sm hover:shadow-2xl transition-all duration-500 border-2 border-rose-500/10 relative text-left h-full flex flex-col" x-data="countdown('{{ $product->flash_sale_end->toIso8601String() }}', {{ $product->stock }})" x-init="init()">
                <a href="{{ route('products.show', $product->slug) }}" class="flex-1">
                    <div class="relative overflow-hidden rounded-[1.5rem] mb-4 aspect-square">
                        <img loading="lazy" src="{{ $product->image_path ? asset('storage/' . $product->image_path) : 'https://via.placeholder.com/300' }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                        <div class="absolute top-3 left-3">
                            <span class="bg-rose-500 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">Limited Time</span>
                        </div>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div class="group bg-white rounded-[3rem] p-6 shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 flex flex-col md:flex-row items-center gap-8 text-left">
                <div class="w-full md:w-2/5 aspect-square rounded-[2.5rem] overflow-hidden bg-slate-50">
                    <img src="{{ $bundle->image_path ? asset('storage/' . $bundle->image_path) : 'https://via.placeholder.com/300' }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                </div>
                <div class="flex-1">
    ...
    <section class="mb-20 reveal-on-scroll" x-data="{ shown: false }" x-intersect.once="shown = true" :class="{ 'is-visible': shown }">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div class="group bg-white rounded-[2rem] p-4 shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 relative text-left flex flex-col">
                <a href="{{ route('products.show', $product->slug) }}" class="flex-1">
                    <div class="relative overflow-hidden rounded-[1.5rem] mb-4 aspect-square">
                        <img src="{{ $product->image_path ? asset('storage/' . $product->image_path) : 'https://via.placeholder.com/300' }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                        <div class="absolute top-3 left-3">
    ...
    <section class="mb-20 reveal-on-scroll" x-data="{ shown: false }" x-intersect.once="shown = true" :class="{ 'is-visible': shown }">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            ...
            <a href="{{ route('products.index', ['category' => $category->slug]) }}" class="group relative flex flex-col items-center p-8 bg-white rounded-[2rem] border-2 border-slate-50 hover:border-sky-500/30 hover:shadow-xl transition-all duration-300 overflow-hidden text-center">
    ...
    <div x-show="openModal" 
         ...
         class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md" @keydown.escape.window="openModal = false" style="display: none;">
...
<script>
    ...
    return {
        shown: false, // Properti untuk memicu animasi reveal
        openModal: false,
        currentPost: {},
        instagramPosts: [
            { 
                id: 1, 
                image: 'https://images.unsplash.com/photo-1594905662978-03a1a2d7eb24?w=600&auto=format&fit=crop&q=60', 
                testimonial: 'Rakitan PC dari Pitocom memang juara! Performa ngebut, desain rapi.', 
                tags: ['#GamingPC', '#Ryzen5'] 
            },
            { 
                id: 2, 
                image: 'https://images.unsplash.com/photo-1696710257827-75e2e5954059?w=600&auto=format&fit=crop&q=60', 
                testimonial: 'Service Pitocom sangat responsif. PC saya kembali prima!', 
                tags: ['#CustomPC', '#RGBbuild'] 
            },
            { 
                id: 3, 
                image: 'https://images.unsplash.com/photo-1636609168915-d561a1d30e87?w=600&auto=format&fit=crop&q=60', 
                testimonial: 'Mantap! Sesuai ekspektasi dan pengiriman aman.', 
                tags: ['#WorkstationPC', '#IntelCore'] 
            },
            { 
                id: 4, 
                image: 'https://images.unsplash.com/photo-1603481588273-2f908a9a7a1b?w=600&auto=format&fit=crop&q=60', 
                testimonial: 'Komponen lengkap, pilihan terbaik ada di Pitocom.', 
                tags: ['#PCbuild', '#NvidiaGeForce'] 
            },
            { 
                id: 5, 
                image: 'https://images.unsplash.com/photo-1648539356777-6a6d9ba6bcd0?w=600&auto=format&fit=crop&q=60', 
                testimonial: 'PC Gaming saya sekarang bisa melibas semua game berat!', 
                tags: ['#GamingSetup', '#HighPerformance'] 
            }
        ],
        showPost(post) {
